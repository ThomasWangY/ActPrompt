...
from dataset import DatasetHL, prepare_batch_inputs_hl, start_end_collate_hl
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if opt.eval_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            with torch.no_grad():
                ...
                logger.info(f"Eval/HL-{opt.dset_name}-{train_val_dataset.domain}-mAP: {float(scores)} Epoch:{epoch_i+1}")
...
def start_training():
    ...
    from config_hl import TVSUM_SPLITS, YOUTUBE_SPLITS
    ...
    # logger.info(f"Eval/HL-{opt.dset_name}-avg-mAP-key: {float(scores)}")
    tb_writer.close()
