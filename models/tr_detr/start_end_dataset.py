...
from models.tr_detr.span_utils import span_xx_to_cxw
import h5py
import os
...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, data_root, v_feat_dirs, q_feat_dir,
        ...
        if self.use_cache > 0:
            ...
            dset_dir = os.path.join(self.data_root, self.dset_name)
    ...
    def __getitem__(self, index):
        ...
        if self.use_video:
            ...
            if ctx_l > meta['duration']:
                model_inputs["video_feat"] = model_inputs["video_feat"][:meta['duration']]
                ctx_l = meta['duration']
        else:
            ctx_l = self.max_v_l
        ...
        if self.load_labels:
            if self.dset_name == 'tvsum': 
                ...
                model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs["saliency_all_labels"] = \
                            self.get_saliency_labels_all_tvsum(meta["relevant_clip_ids"], meta_label, ctx_l)
            elif self.dset_name == 'charades': 

                # pos_idx = torch.tensor(meta['relevant_clip_ids']) 
                start, end = meta['relevant_windows'][0]

                # 使用numpy的arange生成整数区间
                result = list(np.arange(int(np.ceil(start)), int(np.floor(end))+1))
                if len(result) > 1:
                    result = result[:-1]
                # if "qv" in self.dset_name:
                #     result = [i for i in result if i % 2 == 0]
                pos_idx = torch.tensor(result)
                mask = torch.zeros_like(torch.ones(ctx_l))
                ...
                if "subs_train" not in self.data_path and 'saliency_scores' in meta.keys(): # revise
                    model_inputs["saliency_pos_labels"], model_inputs["saliency_neg_labels"], model_inputs["saliency_all_labels"] = \
    ...
    def get_saliency_labels_all_tvsum(self, rel_clip_ids, labels, ctx_l, max_n=1, add_easy_negative=False):
