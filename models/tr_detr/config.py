    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            if opt.dset_domain is not None:
                opt.results_dir = os.path.join(opt.results_root, "tr_detr", "-".join([opt.dset_name, opt.dset_domain, opt.exp_id, time.strftime("%Y_%m_%d_%H")]))
            else:
                opt.results_dir = os.path.join(opt.results_root, "tr_detr", "-".join([opt.dset_name, opt.exp_id, time.strftime("%Y_%m_%d_%H")]))
            mkdirp(opt.results_dir)
            save_fns = ['models/tr_detr/model.py', 'models/tr_detr/transformer.py']
