...
from models.moment_detr.config import TestOptions
from models.moment_detr.model import build_model
from models.moment_detr.span_utils import span_cxw_to_xx
from models.moment_detr.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs, start_end_collate_mr, prepare_batch_inputs_mr
from models.moment_detr.postprocessing_moment_detr import PostProcessorDETR
...
def eval_epoch_post_processing(submission, opt, gt_data, save_submission_filename):
    ...
    if opt.eval_split_name == "val" or opt.eval_split_name == "test" and opt.dset_name not in ['qvhighlights']:  # since test_public has no GT
    ...
    if opt.nms_thd != -1:
        ...
        if opt.eval_split_name == "val" or opt.eval_split_name == "test" and opt.dset_name not in ['qvhighlights']:
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    for batch in eval_loader:
        query_meta = batch[0]
        if opt.dset_name in ['qvhighlights']:
...
def start_inference(split=None, splitfile=None):
