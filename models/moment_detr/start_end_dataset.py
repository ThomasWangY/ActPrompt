...
from models.moment_detr.span_utils import span_xx_to_cxw
import h5py
import os
    ...
    def __getitem__(self, index):
        ...
        if self.load_labels:
            ...
            if 'saliency_scores' in meta.keys():
                ...
                saliency_idx = torch.where(model_inputs['saliency_scores'])[0]
                if len(saliency_idx) > 0: # revised
...
def start_end_collate_mr(batch):
    ...
    for k in model_inputs_keys:
        if k == "span_labels":
            ...
            continue
        if k in ["saliency_pos_labels", "saliency_neg_labels"]:
            batched_data[k] = torch.LongTensor([[e["model_inputs"][k][0]] for e in batch]) # revised
            continue
