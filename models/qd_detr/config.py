...
class BaseOptions(object):
    ...
    def initialize(self):
        ...
        parser.add_argument("--dset_domain", type=str, default=" ", choices=[" ", "BK", "BT", "DS", "FM", "GA", "MS", "PK", "PR", "VT", "VU"], 
        ...
        parser.add_argument("--max_before_nms", type=int, default=1000)
    ...
    def parse(self, a_feat_dir=None):
        ...
        if isinstance(self, TestOptions):
            ...
            # opt.results_dir = os.path.join(opt.results_root, "-".join([opt.dset_name, ctx_str, opt.exp_id, time.strftime("%Y_%m_%d_%H_%M_%S")]))
            if opt.dset_domain is not None:
                opt.results_dir = os.path.join(opt.results_root, "qd_detr", "-".join([opt.dset_name, opt.dset_domain, opt.exp_id, time.strftime("%Y_%m_%d_%H")]))
            else:
                opt.results_dir = os.path.join(opt.results_root, "qd_detr", "-".join([opt.dset_name, opt.exp_id, time.strftime("%Y_%m_%d_%H")]))
            mkdirp(opt.results_dir)
            save_fns = ['models/qd_detr/model.py', 'models/qd_detr/transformer.py']
