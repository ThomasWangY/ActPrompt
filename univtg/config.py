...
class BaseOptions(object):
    ...
    def parse(self, args=None):
        ...
        if isinstance(self, TestOptions):
            ...
            if opt.dset_type != "hl":
                opt.results_dir = os.path.join(opt.results_root, "univtg", "-".join([opt.dset_name, opt.exp_id, time.strftime("%Y_%m_%d_%H")]))
            else:
                opt.results_dir = os.path.join(opt.results_root, "univtg", opt.dset_name, str(opt.seed))
...
def setup_model(opt):
    ...
    importer = importlib.import_module(opt.model_id)
